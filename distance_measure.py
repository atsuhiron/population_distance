...
def _measure_location_dist(smaller: list[Located], larger: list[Located]) -> np.ndarray:
    ...
    if loc_smaller.ndim == 1:
        dist_matrix = measure_jit.calc_dist_matrix_1d_jit(loc_larger, loc_smaller)
    else:
        dist_matrix = measure_jit.calc_dist_matrix_2d_jit(loc_larger, loc_smaller)
...
def _measure_value_dist(smaller: list[ValuedLocated], larger: list[ValuedLocated]) -> np.ndarray:
    ...
    return measure_jit.calc_dist_matrix_1d_jit(val_larger, val_smaller)
